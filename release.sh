...
echo "[2/3] Tagging v1.1.2 ..."
git tag -f v1.1.2
git -c credential.helper='!gh auth git-credential' push -f origin v1.1.2

echo "[3/3] Creating GitHub release ..."
gh release create v1.1.2 \
  dist/ETNetFuturesExporter.exe \
  ETNetFuturesExporter-mac-arm64.dmg \
  --title "ETNet Futures Exporter v1.1.2" \
  --notes "## ETNet Futures Exporter v1.1.2
...
echo "Release published: https://github.com/LightoninT/etnet-download/releases/tag/v1.1.2"
