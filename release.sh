...
echo "[2/3] Tagging v1.0.6 ..."
git tag -f v1.0.6
git -c credential.helper='!gh auth git-credential' push -f origin v1.0.6

echo "[3/3] Creating GitHub release ..."
gh release create v1.0.6 \
  dist/ETNetFuturesExporter.exe \
  ETNetFuturesExporter-mac-arm64.dmg \
  --title "ETNet Futures Exporter v1.0.6" \
  --notes "## ETNet Futures Exporter v1.0.6
...
echo "Release published: https://github.com/LightoninT/etnet-download/releases/tag/v1.0.6"
