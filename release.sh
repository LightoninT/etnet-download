...
echo "[2/3] Tagging v1.1.4 ..."
git tag -f v1.1.4
git -c credential.helper='!gh auth git-credential' push -f origin v1.1.4

echo "[3/3] Creating GitHub release ..."
gh release create v1.1.4 \
  dist/ETNetFuturesExporter.exe \
  ETNetFuturesExporter-mac-arm64.dmg \
  --title "ETNet Futures Exporter v1.1.4" \
  --notes "## ETNet Futures Exporter v1.1.4
...
echo "Release published: https://github.com/LightoninT/etnet-download/releases/tag/v1.1.4"
